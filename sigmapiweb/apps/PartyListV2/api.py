...
from apps.PartyListV2.models import Party, PartyGuest,
RestrictedGuest, PartyCountRecord
...
def modify_party_count(request, party_id):
    ...
    try:
        ...
        if not party.is_list_closed():
            return HttpResponse('Cannot modify list count '
                                'until the party starts', status=403)
...
def sign_out(request, party_id, party_guest_id):
    ...
    try:
        ...
        return HttpResponse('Requested Party Guest does not exist.',
                            status=404)

    if not party.is_list_closed():
        return HttpResponse("Can't sign out guests before the party starts.",
                            status=403)
...
def sign_in(request, party_id, party_guest_id):
    ...
    if not party.is_list_closed():
        return HttpResponse("Can't sign in guests before the party starts.",
                            status=403)
...
def destroy_guest(request, party_id, party_guest_id):
    ...
    if request.method != 'DELETE':
        return HttpResponse('Endpoint supports DELETE method only.',
                            status=405)
    ...
    if party.is_list_closed():
        return HttpResponse('You cannot delete a guest '
                            'after the party list closes.', status=403)

    try:
        guest = PartyGuest.objects.get(pk=party_guest_id)
    except PartyGuest.DoesNotExist:
        return HttpResponse('The guest you are trying to delete '
    ...
    if (guest.added_by == request.user or
        ...
        return HttpResponse("You do not have permission to delete this guest",
                            status=401)
...
def get_restricted_guests(request, party_id):
    response = {
        "restrictedGuests": [guest.to_json() for guest
                             in RestrictedGuest.objects.all()]
    }
    return json_response(response)
...
def get_permissions(request, party_id):
    ...
    response = {
        "permissions": {
            ...
            request.user.has_perm("PartyListV2.can_destroy_any_party_guest"),
            "canAddPartyGuests":
            request.user.has_perm("PartyListV2.add_party_guests"),
            "youHaveDoorAccess":
            request.user.has_perm("PartyListV2.door_access"),
        }
    }
    return json_response(response)
...
def create_guest(request, party_id):
    ...
    if guest_gender is None or not (guest_gender is "M" or
                                    guest_gender is "F"):
    ...
    # Retrieve the brother selected from the dropdown box
    # (either borrowed invite or vouching)
    selected_brother_username = request.POST.get('selectedBrother')
    if selected_brother_username is not None and
    not selected_brother_username.strip() == "":
        try:
    ...
    if not party.is_list_closed():
        ...
        if limit_check:
            return limit_check  # User reached invite limit, return the message

        if selected_brother is not None and
        selected_brother.id == request.user.id:
            return HttpResponse('You cannot borrow an invite '
                                'from yourself (nice try).', status=403)
        ...
        if selected_brother is None:
            return HttpResponse('You must list a vouching brother '
                                'when the list is in party mode.', status=400)

        # Force Add indicates we have already gone through
        # the confirmation modal on client side
        risk_approval = request.POST.get("riskApproval") == "true"

        if party.user_reached_vouching_limit(selected_brother) and
        not risk_approval:
            return HttpResponse('The vouching brother has reached their vouch '
                                'limit, you will need the risk manager'
                                ' or president to authorize this addition.',
                                status=403)

        if not request.user.has_perm("PartyListV2.door_access"):
            return HttpResponse('You do not have permission to create guests'
                                'after the list has closed.', status=403)
...
def __check_invite_limit(preparty_access: bool, party: Party,
                         selected_brother: User, request: HttpRequest):
    if preparty_access:
        # Check to make sure the user hasn't reached their limit
        # (and the person they are borrowing invites from hasn't either)
        if party.has_preparty_invite_limits:
            if selected_brother is None and
            party.user_reached_preparty_limit(request.user):
                return HttpResponse("You have reached your "
                                    "pre-party invite limit.", status=403)
            elif selected_brother is not None and
            party.user_reached_preparty_limit(selected_brother):
                return HttpResponse("The brother you are trying to borrow "
                                    "invites from has "
                                    "reached their pre-party invite limit.",
                                    status=403)
        elif party.has_party_invite_limits:
            if selected_brother is None and
            party.user_reached_party_limit(request.user):
                return HttpResponse("You have pre-party invites left, but your"
                                    " total amount of guests has reached the "
                                    "limit for this party. Try removing a "
                                    "regular guest before adding another "
                                    "to pre-party.", status=403)
            elif selected_brother is not None:
                if party.user_reached_party_limit(selected_brother):
                    return HttpResponse("The brother you are trying to borrow "
                                        "invites from has reached their party "
                                        "invite limit.", status=403)
                elif party.user_reached_preparty_limit(selected_brother):
                    return HttpResponse("The brother you are trying to borrow "
                                        "invites from has reached their "
                                        "pre-party invite limit.", status=403)
    else:
        if party.has_party_invite_limits:
            ...
            party.user_reached_party_limit(request.user):
                return HttpResponse("You have reached your "
                                    "party invite limit.", status=403)
            elif selected_brother is not None and
            party.user_reached_party_limit(selected_brother):
                return HttpResponse("The brother you are trying to borrow "
                                    "invites from has reached their party "
                                    "invite limit.", status=403)
