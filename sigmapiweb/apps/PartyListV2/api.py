...
@login_required
def upgrade_preparty(request, party_id, party_guest_id):
    if request.method != 'POST':
        return HttpResponse('Endpoint supports POST method only.', status=405)
    ...
    try:
        ...
        return HttpResponse('Requested PartyGuest ID does not exist', status=404)

    # TODO(Tom): Expand this to use the inviteUsed feature
    limit_error = __check_invite_limit(True, party, None, request)

    if limit_error:
        return limit_error

    if guest.added_by != request.user:
        return HttpResponse("Only the person who invited this guest can add them to preparty", status=401)

    guest.has_preparty_access = True
    guest.save()

    party.guest_update_counter += 1

    return JsonResponse({'prepartyAccess': guest.has_preparty_access})



@login_required
def downgrade_preparty(request, party_id, party_guest_id):
    if request.method != 'POST':
        return HttpResponse('Endpoint supports POST method only.', status=405)
    ...
    if guest.added_by != request.user:
        return HttpResponse("You do not have permission to remove this guest from preparty", status=401)

    guest.has_preparty_access = False
    guest.save()
